...
from medical_leaves.serializers import MedicalLeaveRecordSerializer

from .models import MedicalLeaveRecord, PatientProfile


class CreateRecordView(generics.CreateAPIView):
    """Create a new record in the system."""
    serializer_class = MedicalLeaveRecordSerializer
...
class ManageRecordView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not MedicalLeaveRecord.objects.filter(id=record_id).exists():
            return Response({"error": "Record not found."},
                            status=status.HTTP_404_NOT_FOUND)
        instance = MedicalLeaveRecord.objects.get(id=record_id)
